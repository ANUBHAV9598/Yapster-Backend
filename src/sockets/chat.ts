...
export const chatSocket = (io: Server) => {
    ...
    const markUserOffline = (socketId: string) => {
    ...
    io.on("connection", (socket: Socket) => {
        ...
        socket.on("register_user", ({ userId }: RegisterUserPayload) => {
        ...
        socket.on("unregister_user", ({ userId }: UnregisterUserPayload) => {
        const normalizedUserId = String(userId ?? "").trim();
        ...
        socket.on("join_chat", ({ chatId }: JoinChatPayload) => {
        if (chatId) {
            socket.join(chatId);
        }
        });

        socket.on("leave_chat", ({ chatId }: JoinChatPayload) => {
        if (chatId) {
            socket.leave(chatId);
        }
        });
      ...
      const sender = isPopulatedUser(populatedMessage.senderId)
        ...
        socket.on("mark_messages_read", async (payload: ReadMessagesPayload) => {
        const chatId = String(payload?.chatId ?? "").trim();
