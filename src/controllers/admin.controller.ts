...
export const deleteAdminUser = async (req: Request, res: Response) => {
  ...
  for (const chat of affectedChats) {
    ...
    if (chat.isGroup && (!chat.admins || chat.admins.length === 0)) {
      chat.admins = [chat.members[0] as mongoose.Types.ObjectId];
    }
...
export const getAdminChats = async (_req: Request, res: Response) => {
...
export const createAdminChat = async (req: Request, res: Response) => {
  ...
  const chat = await Chat.create({
    ...
    admins: finalAdminIds as unknown as mongoose.Types.ObjectId[],
  });
...
export const updateAdminChat = async (req: Request, res: Response) => {
  const chatId = String(req.params.chatId ?? "").trim();
  const title = typeof req.body.title === "string" ? req.body.title.trim() : undefined;
  const memberIds = normalizeIds(req.body.memberIds);
  const adminIds = normalizeIds(req.body.adminIds);
  const isGroup =
    typeof req.body.isGroup === "boolean" ? req.body.isGroup : undefined;
  ...
  if (adminIds.length > 0 || chat.isGroup) {
    ...
    const validAdminIds = adminIds.filter((adminId) =>
      (memberIds.length > 0 ? memberIds : chat.members.map((member) => String(member))).includes(adminId),
    );

    chat.admins = validAdminIds as unknown as mongoose.Types.ObjectId[];
  }
...
export const deleteAdminChat = async (req: Request, res: Response) => {
  const chatId = String(req.params.chatId ?? "").trim();
...
export const getAdminMessages = async (_req: Request, res: Response) => {
